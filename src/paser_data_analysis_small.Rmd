...
paser_results_small <- "C:\\Users\\BhatA\\Box\\ASMS\\paser_small_lib.tsv"

paser_small_corr <- "C:\\Users\\BhatA\\Box\\ASMS\\sample_matrix_small_20on20_withCorr.tsv"
```
...
sample_matrix_small_20on20 <- filter(sample_matrix_small,!Protein %in% sample_matrix_small_cv_filt$Protein)

sample_matrix_small_20on20 <- sample_matrix_small_20on20 %>%
  group_by(Protein) %>%   
  summarise(Precursor.Quantity = n())
...
write.table(sample_matrix_small_20on20_final, file = "sample_matrix_small_20on20_filtCV.tsv", sep="\t", row.names=F, quote=F) # export to file
...
# Filtered
summarize_precursor_filt <- sample_matrix_small_20on20_final %>% group_by(Category) %>% summarise(Count = n_distinct(Protein))
