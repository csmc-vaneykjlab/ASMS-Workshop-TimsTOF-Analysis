...
paser_big_corr <- "C:\\Users\\BhatA\\Box\\ASMS\\sample_matrix_big_withCorr.tsv"
```
...
sample_matrix_big <- filter(sample_matrix_big, Identified.By != "MBR")
...
sample_matrix_big_20on20 <- sample_matrix_big %>%
  group_by(Protein) %>%   
  summarise(Precursor.Quantity = n())
...
write.table(sample_matrix_big_20on20_final, file = "sample_matrix_big_filt20on20.tsv", sep="\t", row.names=F, quote=F) # export to file
...
sample_matrix_big_cv_filt_subset <- filter(sample_matrix_big_20on20_final,!Protein %in% sample_matrix_big_cv_filt$Protein)

# save table with CV < 20%
write.table(sample_matrix_big_cv_filt_subset, file = "sample_matrix_big_filtCV.tsv", sep="\t", row.names=F, quote=F) # export to file
...
# Filtered 
summarize_precursor_filt <- sample_matrix_big_cv_filt_subset %>% group_by(Category) %>% summarise(Count = n_distinct(Protein))
...
# Filtered
median_grp_precursor_filt = sample_matrix_big_cv_filt_subset %>% group_by(Category, Protein) %>% summarise(Precursor.Quantity = median(Precursor.Quantity))
...
# Filtered
cv_precursor_filt <- sample_matrix_big_cv_filt_subset %>% 
    group_by(Category,Protein) %>%
    summarise(StdDev = sd(Precursor.Quantity), Avg = mean(Precursor.Quantity), CV = (StdDev/Avg)*100)
```
...
# Filtered
rt_precursor_filt <- sample_matrix_big_cv_filt_subset %>% 
    group_by(Category,Protein) %>%
    summarise(Median = median(RT))
```
...
# Filtered
im_precursor_filt <- sample_matrix_big_cv_filt_subset %>% 
    group_by(Category,Protein) %>%
    summarise(Median = median(Exp.1.K0))
```
